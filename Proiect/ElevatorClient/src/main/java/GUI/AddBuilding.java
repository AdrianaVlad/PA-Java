...
public class AddBuilding extends JPanel{
    ...
    public AddBuilding(MainFrame frame){
        this.frame=frame;
        init();
    }
    private void init(){
        statement = new JLabel("Please add building details:");
        ...
        add(back);
        if(failedAttempt){
            noticeLabel = new JLabel("That name is already taken!");
    ...
    public void submit(ActionEvent e){
        ...
        if(!buildingFound){
            frame.comms.createBuilding(name);
            toSelect(e);
        }
        else{
            failedAttempt = true;
    ...
    public void toSelect (ActionEvent e){
        ...
        frame.login.moveToSelectBuilding(4);
    }
}
