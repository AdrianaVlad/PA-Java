        ...
        try {
            ...
            while(running){
                ...
                if(game!=null && game.outOfTime(playerNr) && game.isTurn(playerNr)){
                    server.gameController.deleteGame(game.getName());
                    ...
                    if(request.equals("create tournament")){
                        if(game!=null){
                            ...
                            if(n==0)
                                n=server.playerController.countPlayers();
                        ...
                        if(game!=null)
                            ...
                            out.flush();
                            request = in.readLine();
                            game = server.gameController.findByName(request);
                            if(game==null)
                        ...
                        response="Select a new name:";
                        out.println(response);
                        out.flush();
                        request = in.readLine();
                        while(server.playerController.updatePlayer(player.id, request).getStatusCode()==HttpStatus.NOT_FOUND){
                            out.println("Name taken, try a different one:");
                            out.flush();
                            request = in.readLine();
                        }
            ...
            try {
                socket.close();
                server.playerController.deletePlayer(player.id);
            } catch (IOException e) {
